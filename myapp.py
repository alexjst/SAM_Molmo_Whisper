...
with gr.Blocks(
    ...
    # Removed: txt_input and audio_input

    # Outputs.
    img_plt_out = gr.Plot(label='Segmentation Result', format='png')
    # Removed: molmo_out, whisper_out, and pointed_image

    # Removed: img_input.select(get_click_coords, [img_input], [pointed_image])

    # Additional inputs - hidden but still needed for functionality
    ...
    gr.Interface(
        ...
        description=f"Upload an image and segment specific objects in the image.",
        allow_flagging="never",  # Removes flag button
        submit_btn="Segment",    # Changes Submit to Segment
        clear_btn="Reset"        # Changes Clear to Reset
    )
